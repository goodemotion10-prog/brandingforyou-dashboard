  ...
  try {
        ...
        const adData = await adRes.json();
          if (!adData.keywordList) {
                      return NextResponse.json({ error: 'Failed to fetch keywords from Naver Ads' }, { status: 500 });
          }

        const keywords = adData.keywordList.slice(0, 10);

        const results = await Promise.all(keywords.map(async (item) => {
                  ...
                  const blogCount = searchData.total || 0;
                                                             ...
                                                             return {
                                                                         ...
                                                                         searchVolume: vol,
                                                                         blogCount,
                                                                         ratio: vol > 0 ? (blogCount / vol).toFixed(2) : 0,
                                                             };
        }));
